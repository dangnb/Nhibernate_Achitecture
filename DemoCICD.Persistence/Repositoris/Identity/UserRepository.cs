...
public class UserRepository : Repository<Userdata, int>, IUserRepository
{
    public UserRepository(string sessionFactoryConfigPath, string connectionString = null) : base(sessionFactoryConfigPath, connectionString)
    {
    }
    ...
    public Userdata Authenticate(string groupcode, string username, string password)
    {
        ...
        string hashPassword = PasswordUtil.EncodePassword(password, user.PasswordSalt);
    ...
    {
        return Get(p => p.GroupCode == groupcode && (p.Username == username || p.Email == username));
    }

    public bool ChangePassword(Userdata userdata, string password, string newpassword, out string message)
    {
        message = "";
        string hashPassword = PasswordUtil.EncodePassword(password, userdata.PasswordSalt);
        if (userdata.Password != hashPassword)
        {
            message = "Mật khẩu không hợp lệ";
            return false;
        }
        string hashNewPassword = PasswordUtil.EncodePassword(newpassword, userdata.PasswordSalt);
